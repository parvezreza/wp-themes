...
function titan_option_metabox(){
    $titan = TitanFramework::getInstance("tf");
    ////////// Options Fields /////////////
    $adminPanel = $titan->createAdminPanel( array(
        'name' => 'Theme Options',
    ) );
    $defautl = $adminPanel->createTab( array(
        'name' => 'Default',
    ) );
    $defautl->createOption(array(
        "name"=>"Call Us",
        "type"=>"text",
        "id"=>"call_us",
        "desc"=>""
    ...
    $header->createOption(array(
        ...
        "type"=>"text",
        "id"=>"header_logo_text",
        "desc"=>"Specialized Power Systems Supplies"
    ));
    $header->createOption(array(
        "name"=>"Facebook",
        "type"=>"text",
    ...
    //Google Map///
    $support_bottom = $adminPanel->createTab( array(
        'name' => 'Google Map',
    ) );
    $support_bottom->createOption(array(
        "name"=>"Map",
        "type"=>"text",
        "id"=>"google_map",
    ));
...
add_action("tf_create_options","titan_option_metabox");
//Login Failed then not redirect wp_login page
...
function cubiq_registration_redirect ($errors, $sanitized_user_login, $user_email) {
    if ( !empty( $errors->errors) ) {
        if ( isset( $errors->errors['username_exists'] ) ) {
            ...
            wp_redirect( get_the_permalink(32) . '?action=register&failed=username_exists' );

        } else if ( isset( $errors->errors['email_exists'] ) ) {

            wp_redirect( get_the_permalink(32) . '?action=register&failed=email_exists' );

        } else if ( isset( $errors->errors['empty_username'] ) || isset( $errors->errors['empty_email'] ) ) {

            wp_redirect( get_the_permalink(32) . '?action=register&failed=empty' );

        } else if ( !empty( $errors->errors ) ) {

            wp_redirect( get_the_permalink(32) . '?action=register&failed=generic' );
